...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($input['title']) && isset($input['amount']) && isset($input['type']) && isset($input['category'])) {
        ...
        $sql = "INSERT INTO transactions (title, amount, type, category_id, date) VALUES ('$title', $amount, '$type', $categoryId, '$date')";
        
        if ($conn->query($sql) === TRUE) {
            echo json_encode(array("success" => true, "message" => "Transaction created successfully", "id" => $conn->insert_id));
        } else {
            http_response_code(500);
